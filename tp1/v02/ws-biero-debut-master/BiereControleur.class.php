...
class BiereControleur 
{
		...
		if(!$this->valideAuthentification())
		{
			...
			if(isset($oReq->url_element[1]) && is_numeric($oReq->url_element[1]))//Route : /biere/:id/..
			{
				...
				if(isset($oReq->url_element[2])) //Route : biere/:id/{note, commentaire}
				{
					switch ($oReq->url_element[2]) {
						case 'note':
							$courriel = $oReq->parametres["courriel"]; //??
							$oUsager = new Usager ();
							$id_usager = $oUsager->getUsagerParCourriel($courriel);
							
							$id_biere = $oReq->url_element[1];
							
							$note = $oReq->parametres["note"]; // ??
							
							$oComm = new Note ();
							$res = $oComm->ajouterNote($id_usager, $id_biere, $note);
							break;
						case 'commentaire':
								//$res = $this->getCommentaire($id_biere);
	...
	private function getBiere($id_biere)
	{
		...
		$oNote = new Note();
		$resNote = $oNote->getNombre($id_biere);
